// Multi-channel convolution benchmark: custom naive vs cuDNN (FP32), validated vs CPU.
...
static void check(float* d_out, float* h_gpu, const float* h_cpu, size_t outN, const char* name) {
...
static int argi(int argc, char** argv, const char* key, int defv) {
    for (int i = 1; i < argc; ++i)
        if (strncmp(argv[i], key, strlen(key)) == 0) return atoi(argv[i] + strlen(key));
    return defv;
}

int main(int argc, char** argv) {
    const int N = 1;
    int Cin=argi(argc,argv,"--cin=",16), Cout=argi(argc,argv,"--cout=",16);
    int H=argi(argc,argv,"--hw=",256), W=H;
    const int R=3, S=3; int Hout=H-R+1, Wout=W-S+1;
    ...
    CHECK_CUDA_ERR(cudaMalloc(&d_in,inN*sizeof(float)));   CHECK_CUDA_ERR(cudaMalloc(&d_filt,filtN*sizeof(float)));
    CHECK_CUDA_ERR(cudaMalloc(&d_naive,outN*sizeof(float)));CHECK_CUDA_ERR(cudaMalloc(&d_cudnn,outN*sizeof(float)));
    ...
    cudnn_convolution_setup(cudnn,inDims,filtDims,outDims,0,0,1,1,&inDesc,&filtDesc,&convDesc,&outDesc,&algo,&ws,&wsSize);
    ...
    printf("  cuDNN : %8.4f ms   (naive/cuDNN = %.2fx)\n", ms_cudnn, ms_naive/ms_cudnn);
